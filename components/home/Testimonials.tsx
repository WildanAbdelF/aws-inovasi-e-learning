    ...
    <section className="bg-neutral-100 py-12 md:py-20">
      <div className="max-w-6xl mx-auto px-4 md:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mt-8 md:mt-12">
          {data.map((t, i) => (
            <div key={i} className="bg-white p-4 md:p-6 rounded-xl shadow">
              <p className="italic text-neutral-700 text-sm md:text-base">{t.text}</p>
              <p className="font-semibold mt-4 text-center text-sm md:text-base">{t.name}</p>
            </div>
          ))}
        </div>
      </div>
